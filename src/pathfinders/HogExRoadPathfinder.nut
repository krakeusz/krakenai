...
/**
 * A Road Pathfinder based on AIHogEx's Road PathFinder.
...
class HogExRoadPathFinder
{
	...
	_max_cost = 10000000;              ///< The maximum cost for a route.
	_cost_tile = 50;             ///< The cost for a single tile.
	_cost_no_existing_road = 140; ///< The cost that is added to _cost_tile if no road exists yet.
	_cost_turn = 50;             ///< The cost that is added to _cost_tile if the direction changes.
	_cost_slope = 50;            ///< The extra cost if a road tile is sloped.
	_cost_bridge_per_tile = 100;  ///< The cost per tile of a new bridge, this is added to _cost_tile.
	_cost_tunnel_per_tile = 100;  ///< The cost per tile of a new tunnel, this is added to _cost_tile.
	_cost_coast = 50;            ///< The extra cost for a coast tile.
	_cost_drivethroughstation = 1000;
	_cost_level_crossing = 1000;
	_cost_demolish_tile = 1000;
	_pathfinder = null;            ///< A reference to the used AyStar object.
	_max_bridge_length = 10;     ///< The maximum length of a bridge that will be build.
	_max_tunnel_length = 10;     ///< The maximum length of a tunnel that will be build.
	_estimate_rate = 2;
...
class HogExRoadPathFinder.Cost
{
	_main = null;
...
function HogExRoadPathFinder::FindPath(iterations)
{
...
function HogExRoadPathFinder::_GetBridgeNumSlopes(end_a, end_b)
{
	local slopes = 0;
...
function HogExRoadPathFinder::DebugSign(tile,text) {
	if(debug) {
...
function HogExRoadPathFinder::_Cost(self, path, new_tile, new_direction)
{
...
function HogExRoadPathFinder::_Estimate(self, cur_tile, cur_direction, goal_tiles)
{
...
function HogExRoadPathFinder::_Neighbours(self, path, cur_node)
{
	...
	if ((AIBridge.IsBridgeTile(cur_node) || AITunnel.IsTunnelTile(cur_node)) &&
		...
		if (self._AreRoadTilesConnected(cur_node, next_tile) || AITile.IsBuildable(next_tile) || AIRoad.IsRoadTile(next_tile)) {
			tiles.push([next_tile, self._GetDirection(cur_node, next_tile, false), pathMode]);
		}
		/* The other end of the bridge / tunnel is a neighbour. */
		tiles.push([other_end, self._GetDirection(next_tile, cur_node, true) << 4, pathMode]);
		...
		foreach (offset in HogExRoadPathFinder.DIR4Index) {
			...
			if (self._AreRoadTilesConnected(cur_node, next_tile)) {
				...
				if(AIRoad.BuildRoad(cur_node, next_tile)) {
					tiles.push([next_tile, self._GetDirection(cur_node, next_tile, false), pathMode]);
				} else if( !AIRoad.IsRoadDepotTile(cur_node)
		...
		if (path.GetParent() != null) {
			local bridges = self._GetTunnelsBridges(path.GetParent().GetTile(), cur_node, self._GetDirection(path.GetParent().GetTile(), cur_node, true) << 4, path, pathMode);
...
function HogExRoadPathFinder::IsTownBuilding(tile) {
...
function HogExRoadPathFinder::IsBusyRoad(tile) {
...
function HogExRoadPathFinder::CheckRunnableRoadTypes() {
	runnableRoadTypes = this._GetHasPowerRoadTypes(engine);
	foreach(roadType in runnableRoadTypes) {
		AILog.Info("runnableRoadType:"+AIRoad.GetName(roadType)+" "+AIEngine.GetName(engine));
	}
}

function HogExRoadPathFinder::_AreRoadTilesConnected(cur, next) {
	if(!AIRoad.AreRoadTilesConnected(cur, next)) {
		return false;
	}
	foreach(roadType in this.runnableRoadTypes) {
...
function HogExRoadPathFinder::_CheckDirection(self, tile, existing_direction, new_direction)
{
	return false;
}

function HogExRoadPathFinder::_GetDirection(from, to, is_bridge)
{
...
function HogExRoadPathFinder::_GetTunnelsBridges(last_node, cur_node, bridge_dir, path, mode) {
	...
	for (local i = 2; i < this._max_bridge_length; i++) {
		...
		if (!bridge_list.IsEmpty() && !_goals.HasItem(target) &&
			...
			tiles.push([target, bridge_dir, mode]);
		}
	}
	...
	if (AITunnel.GetOtherTunnelEnd(other_tunnel_end) == cur_node && tunnel_length >= 2 &&
		...
		tiles.push([other_tunnel_end, bridge_dir, mode]);
	}
	return tiles;
}

function HogExRoadPathFinder::_IsSlopedRoad(start, middle, end)
{
...
function HogExRoadPathFinder::_CheckTunnelBridge(current_tile, new_tile)
{
...
function HogExRoadPathFinder::_GetHasPowerRoadTypes(engine) {
	local result = [];
